...
def llama_new_forward(
    ...
    if use_attn and not use_cfg:
        attn_weights[:, :, -1, img_start_idx:img_end_idx] = (
            self.alpha * (attn_weights[:, :, -1, img_start_idx:img_end_idx].abs() * (1 - self.beta) + self.avg_attn_weights.squeeze(2) * self.beta) # fusion current attention pattern and average pattern of previous steps
            + attn_weights[:, :, -1, img_start_idx:img_end_idx]
        )
    ### PAI's modification
